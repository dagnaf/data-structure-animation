...
function _draw_nodes () {
  var data = status.pns.filter(function (d) { return !isNaN(d.w); });
  ...
  function _visited(d,i) { return status.pns[i] && status.pns[i].w === status.neg; }
  ...
  gprimtext.selectAll('text.dist').data(status.pns, _fnid)
    .attr("transform", _transform)
    .text(function (d) { return d.w === status.neg ? '-∞' : (d.w === status.pos ? '+∞' : d.w); })
  gprimtext.selectAll('text.dist').data(status.pns, _fnid)
    .exit().remove();
  // hl
  gnodes.selectAll("circle.node")
...
function selected(d) {
  return status.se.concat(status.pns.filter(function (d) { return d.w !== status.neg }).map(function (d) { return d.e; })).some(function (se) {
  // return status.se.some(function (se) {
    return se && sameEdge(d,se);
  })
}
function reversed(d) {
  return status.se.concat(status.pns.filter(function (d) { return d.w !== status.neg }).map(function (d) { return d.e; })).some(function (se) {
