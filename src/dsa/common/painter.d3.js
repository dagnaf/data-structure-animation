...
function init (opt) {
  ...
  graph.nodes = opt.nodes || d3.range(opt.n || 5).map(function () { return {}; })
  graph.edges = (opt.edges || d3.range(5).map(function (d) {
    return { u: d, v: (d+1) % 5 };
  })).map(function (d) { return {
    source: graph.nodes[d.u], target: graph.nodes[d.v], val: d.w,
    reflexive: d.reflexive
  }});

  selected = undefined;
  _onSelected = opt._onSelected || undefined;
  _onLoad = opt._onLoad || undefined;
  source = undefined;
  dragging = false;
  ...
  var defs = d3.select(svgDom).append("defs");
  defs.append("marker")
      .attr("id", "arrow-end")
...
function link(d) {
  ...
  var target = d3.interpolate(d.target, d.source)(radius/dr);
  var ang = d3.interpolate(30,0)(radius/dr);
  // dx *= radius/dr
  // dy *= radius/dr
  ...
  if (directional && weighted) {
    ...
    if (redge.length > 0) {
      d.curve = true;
      source = translate(rotate(translate(source, d.source), ang), neg(d.source));
      target = translate(rotate(translate(target, d.target), -ang), neg(d.target));
...
function convert(_directional, _weighted) {
  ...
  graph.edges = edges;
  gedges.classed('directional', directional)
...
module.exports = {
  ...
  clear: clear,
  random: random,
  directional: function () { return directional; },
  weighted: function () { return weighted; },
  raw: function () { return graph; },
  data: function () {
    var e = graph.edges.map(function (d) {
      return { u: d.source.index, v: d.target.index, w: d.val };
    });
    if (!directional) {
      e = e.concat(graph.edges.map(function (d) {
        return { u: d.target.index, v: d.source.index, w: d.val };
      }));
    }
    if (directional && !weighted) {
      e = e.concat(graph.edges.filter(function (d) {
        return d.reflexive;
      }).map(function (d) {
        return { u: d.target.index, v: d.source.index, w: d.val };
      }));
